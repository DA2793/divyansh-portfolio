  ...
  return (
    <section id="experience" className="py-24 bg-[#f7f6f2] text-[#0f172a]">
      <div className="max-w-5xl mx-auto px-6">
        ...
        {/* Staggered layout — Amazon wider left, iEnergizer narrower right */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">

          {/* ─── AMAZON (7 cols) ─── */}
          <div className="md:col-span-7 bg-white rounded-2xl p-8 border border-gray-100 shadow-sm">
            <div className="flex items-start justify-between gap-4 mb-2">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#475569]">Supply Chain Operations Manager</p>
                </div>
              </div>
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              {amazonImpact.map(({ value, label }) => (
                <div key={label} className="bg-[#f7f6f2] rounded-xl px-4 py-2.5 border border-gray-100 flex items-center gap-2.5">
            ...
            <ul className="space-y-2.5 mb-6">
              {amazonBullets.map((item) => (
                <li key={item} className="flex items-start gap-3 text-sm text-[#475569] leading-relaxed">
          ...
          {/* ─── IENERGIZER (5 cols) ─── */}
          <div className="md:col-span-5 bg-white rounded-2xl p-8 border border-gray-100 shadow-sm">
            <div className="flex items-start justify-between gap-4 mb-2">
              <div className="flex items-center gap-4">
                <img src="/ienergizer.png" alt="iEnergizer" className="w-10 h-10 object-contain rounded-lg" />
                <div>
                  <h3 className="text-lg font-bold text-[#0f172a]">iEnergizer</h3>
                  <p className="text-sm text-[#475569]">Credit One Bank – US Credit Card Issuer</p>
                </div>
              </div>
            </div>
            ...
            <span className="text-xs text-gray-400 font-medium">2015 – 2021</span>

            {/* Role progression */}
            <div className="relative pl-5 border-l-2 border-gray-100 mt-4 mb-6 space-y-4">
              {iEnergizerProgression.map(({ role, period, active }) => (
                <div key={role} className="relative">
                  <span
                    className={`absolute -left-[21px] top-1 w-2.5 h-2.5 rounded-full border-2 ${
                      active ? "bg-[#0f172a] border-[#0f172a]" : "bg-white border-gray-300"
                    }`}
                  />
                  <div className="flex items-center justify-between">
                    <p className={`text-sm font-semibold ${active ? "text-[#0f172a]" : "text-gray-400"}`}>
                      {role}
                    </p>
